...
class CausalAdditiveNoSpurious(Envs):
  def __init__(self, d_x_z_perp = 1, d_x_y_perp = 1, d_u = 1, d_x_y = 1, guassian_normalized_weight = False):
    ...
    if guassian_normalized_weight:
      ...
      self.w_x_z_perp = np.random.uniform(low = -3, high = 3, size=(d_x_z_perp, 1)) 
    
    #input_dim 
    self.input_dim = self.d_x_z_perp + self.d_x_y_perp + self.d_x_y

    self.env_means = [0.1, 5, 0.3, 1]
  ...
  def sample_envs(self, env_ind, n = 100):
    ...
    x_z_perp = np.random.uniform(low = -3, high = 3, size=(n, self.d_x_z_perp))
    u = np.random.uniform(low = -3, high = 3, size=(n, self.d_u))  / self.env_means[env_ind]


    # + self.env_means[env_ind]
    ...
    # w_u[0,0] = self.env_means[env_ind]
    y = self.fn_y(x_z_perp, self.w_x_z_perp, u, w_u) #+ x_y_perp

    if self.d_x_y != 0:
      x_y = y * self.env_means[env_ind] + np.random.randn(n, self.d_x_y) * 0.1
  ...
  def phi_x_y_perp(self, x):
    n = x.shape[0]
    return 1 / (1 + np.exp(-x)) + np.random.randn(n, x.shape[1])*0.1
...
class AntiCausal(CausalAdditiveNoSpurious):
  ...
  def sample_envs(self, env_ind, n = 100):
    ...
    x_u_perp = self.phi_base(y) + np.random.randn(n, self.d_u_perp)*0.1
